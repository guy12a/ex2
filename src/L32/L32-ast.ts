...
;;         |  ( dict (<dictBinding>*))      / DictExp(pairs:DictBinding[])
;; <binding>  ::= ( <var> <cexp> )          / Binding(var:VarDecl, val:Cexp)
;; <dictBinding> ::= ( <symbol> <cexp>)        / DictBinding(var:symbol, val:Cexp)
...
export type DictExp = {tag: "DictExp"; pairs: DictBinding[]}
export type DictBinding = {tag: "DictBinding"; var: SymbolSExp; val: CExp; }
...
export const makeDictExp = (pairs: DictBinding[]) : DictExp =>
    ({tag: "DictExp", pairs: pairs});
export const makeDictBindingExp = (v: SymbolSExp, val: CExp): DictBinding =>
    ({tag: "DictBinding", var: v, val: val});
...
export const isDictBindingExp = (x:any): x is DictBinding => x.tag === "DictBinding";
...
const parseDictExp = (pairs: Sexp[]): Result<DictExp> => {
    if (!allT(isGoodPair, pairs)) {
        return makeFailure('Malformed bindings in "dict" expression');
    }
    
    const vars = map(b => b[0], pairs);
    if (!isGoodVars(vars)) {
        return makeFailure("Duplicate keys found in dict");
    }
    const symbols = map(makeSymbolSExp, vars);


    const valsResult = mapResult(parseL32CExp, map(second, pairs));
    const bindingsResult = mapv(valsResult, (vals: CExp[]) => zipWith(makeDictBindingExp, symbols, vals));

    return bind(bindingsResult, (bindings: DictBinding[]) => makeOk(makeDictExp(bindings)));
}

const isGoodPair = (pair: Sexp): pair is [string, Sexp] =>
    isArray(pair) &&
    pair.length===2 &&
    typeof pair[0] === "string";

const isGoodVars = (vars: string[]): boolean => {
    const uniqueVars = new Set(vars);    // Create a Set to keep unique values
    // If the number of unique keys is the same as the original list, no duplicates exist
    return uniqueVars.size === vars.length;
}
...
const unparseDictExp = (di: DictExp) : string =>
    `(dict (${map((b: DictBinding) => `(${b.var.val} ${unparseL32(b.val)})`, di.pairs).join(" ")}))`
