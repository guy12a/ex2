...
import { Binding, CompoundExp, DictBinding, DictExp, isCExp, isDictExp, isLetExp, makeIfExp } from "./L32-ast";
...
import { isClosure, makeClosure, Closure, Value, makeCompoundSExp, makeSymbolSExp, makeEmptySExp, CompoundSExp, SExpValue, EmptySExp, SymbolSExp, isSymbolSExp, isEmptySExp, isCompoundSExp, makeDictValue, isDictValue, DictValue } from "./L32-value";
...
//Evaluating dict: we get (dict (a 1) (b 2) ... (key: symbol, val:Cexp))
//each of these pairs, we evaluate the val - and then create an evaluated pair (key: symbol, val: value)
//with all these evaluated pairs, we create a DictValue
const evalDict = (exp: DictExp, env: Env): Result<Value> => 
    bind(mapResult((b: DictBinding)=> bind(L32applicativeEval(b.val,env), (val: Value) => 
        makeOk<[SymbolSExp,Value]>([b.var, val])),exp.pairs), (pairs: [SymbolSExp,Value][])=> 
            makeOk(makeDictValue(pairs)));  
...
const applyDict = (proc: DictValue, args: Value[]): Result<Value> =>{
    if(args.length !== 1){
        return makeFailure("Dict got too many or too little args to look for");
    }
    const key = args[0];
    if(!isSymbolSExp(key)){
        return makeFailure("key isnt symbol");
    }

    const entry = proc.pairs.find(([sym, _]) => sym.val === key.val);
    return entry ? makeOk(entry[1]) : makeFailure(`Key '${key.val}' not found`);
}
